import {initTutorial,validateTutorial,tutorialBattle,tutorialSettle,tutorialDamage} from './tutorial.js';
export {TUTORIAL_STAGES,tutorialGrant,tutorialDismiss} from './tutorial.js';
...
if(s.battle!==null){const b=s.battle;if(b.tutorialDamage!==undefined&&(!Number.isFinite(b.tutorialDamage)||b.tutorialDamage<0||b.tutorialDamage>1))bad();if(!obj(b)||!Number.isInteger(b.wind)||Math.abs(b.wind)>WIND_TUNING.maxStrength||!['player','enemy','flight','special','result'].includes(b.phase)||!fighterOK(b.player)||!fighterOK(b.enemy)||!int(b.rng,4294967295)||!int(b.turn,25)||!int(b.shots,2)||!Array.isArray(b.events)||!Array.isArray(b.log)||!bool(b.rewarded)||!bool(b.charged)||!bool(b.used))bad();}
...
export function openChest(s,id){const c=s.chests.find(c=>c.id===id);if(!c)return null;const r=seeded(c.seed),spec=E.CHESTS[c.kind],gains={},draws=[];s.gold+=spec.gold;for(let i=0;i<4;i++){const rarity=roll(r,spec.odds),pool=PIRATES.filter(p=>p.rarity===rarity),p=pool[Math.floor(r()*pool.length)];draws.push(p.id);gains[p.id]=(gains[p.id]||0)+1;if(!s.levels[p.id])s.levels[p.id]=1;if(s.levels[p.id]===12)s.gold+=10;else s.cards[p.id]++;}s.chests=s.chests.filter(x=>x.id!==id);return {gold:spec.gold,cards:gains,draws};}
...
export function startBattle(s,t=Date.now(),rematch=false){if(s.tutorial?.pending)return false;if(!Object.values(s.slots).length)return false;const old=s.lastResult;if(rematch&&(!old||old.won||old.rematch||old.rematchAsked))return false;const seed=rematch?old.seed:Math.floor(t)%2147483647,r=seeded(seed);const b={id:String(t),seed,wind:battleWind(seed),rng:seed,turn:1,shots:2,seconds:30,dualHits:0,streak:0,streakVersion:2,charged:false,used:false,phase:'player',log:[],events:[],rematch,enemyName:rematch?old.enemyName:board(s).filter(p=>!p.self)[Math.floor(r()*99)].name,temperament:['supportive','angry','fun','playful','competitive','rager','silly'][Math.floor(r()*7)],player:fighter(s,false,r),enemy:fighter(s,true,r),started:t,rewarded:false};if(rematch&&old.opponent){b.enemy=clone(old.opponent);b.temperament=old.temperament;}tutorialBattle(s,b);prepareBattle(b);b.opponent=clone(b.enemy);s.battle=b;return b;}
...
function refreshMovingFlight(b,pending,flight,elapsed){
 ...
 const base=shotPower(own,pirate,g)*(pending.side==='enemy'&&!b.practice?(b.tutorialDamage??1):1),remaining=new Set(flight.shots.map((_,i)=>i).filter(i=>!pending.resolved.includes(i)));
 let cursor=from;
 while(remaining.size){
  const candidates=[];
...
export function advanceShot(s,elapsed){
 ...
 const base=shotPower(own,p,g)*tutorialDamage(s,side),fresh=[];pending.elapsed=Math.min(flight.duration,Math.max(pending.elapsed||0,elapsed));
...
initTutorial(s);
if(s.battle&&s.battle.tutorialDamage===undefined&&!s.battle.rewarded)tutorialBattle(s,s.battle);
